...
// src/functions/update-blog/handler.ts
__export(exports, {
  main: () => main
});
...
var PostRepository = class {
  constructor(docClient = createDynamoDBClient(), todoTable = process.env.POST_TABLE) {
...
var handler = async (event) => {
  ...
  const { id } = event.body;
  const blog = { id, author, description, title, content, createdAt: new Date().toLocaleDateString() };
  const blogPartial = __spreadProps(__spreadValues({}, blog), { id });
